...
def output_keypoints( frame,net,proto_file, weights_file, threshold, BODY_PARTS):
    global points
...
#    cos_theta = (vector_a[0] * vector_b[0] + vector_a[1] * vector_b[1]) / ( ((vector_a[0] ** 2 + vector_a[1] ** 2) ** 0.5) * ((vector_b[0] ** 2 + vector_b[1] ** 2) ** 0.5) )+1e-2
    ...
    if abs(cos_theta) > (2 ** 0.5) / 2:
        ...
        cv2.putText(frame, string, (200, 475), cv2.FONT_HERSHEY_DUPLEX, 1, (255, 0, 255))
...
def calculate_degree_waist2(point_1, point_2, frame): # p1 -> 12번, p2 -> 9번
    ...
    if abs(cos_theta) <= (3 ** 0.5) / 2:
        ...
        cv2.putText(frame, string, (350, 475), cv2.FONT_HERSHEY_DUPLEX, 1, (25, 100, 25))
        print(f"[degree] ({string})")
        return 2
...
def calculate_degree_neck(point_1, point_2, point_3, point_4, frame):
    ...
    if abs(cos_theta) > (2 ** 0.5) / 2:
        ...
        cv2.putText(frame, string, (0, 475), cv2.FONT_HERSHEY_DUPLEX, 1, (255, 255, 0))
        print(f"[degree] ({string})")
        return 2
    else:
        string = "Neck-Good"
...
def calculate_degree_shoulder2(point_1, point_2, frame): # p1 -> 5번, p2 -> 2번
    ...
    if abs(cos_theta) <= (3 ** 0.5) / 2:
        ...
        cv2.putText(frame, string, (300, 25), cv2.FONT_HERSHEY_DUPLEX, 1, (255, 100, 255))
        print(f"[degree] ({string})")
        return 2
    else:
